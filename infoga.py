...
class Infoga(object):
	...
	def GetInfo(self):
		for x in range(len(self.AllEmails)):
			...
			for s in range(len(new)):
				...
				res = net.request("GET", "https://api.shodan.io/shodan/host/" + new[
					s] + "?key=UNmOjxeFS2mPA3kmzm1sZwC0XjaTTksy")
	...
	def Getinfo(self, email):
		...
		for s in range(len(new)):
			...
			net = urllib3.PoolManager()
			res = net.request("GET",
			                  "https://api.shodan.io/shodan/host/" + new[s] + "?key=UNmOjxeFS2mPA3kmzm1sZwC0XjaTTksy")
			jso = json.loads(res.data)
			try:
				self.sock = socket.gethostbyaddr(new[s])[0]
			except socket.herror, err:
	...
	def CheckUpdate(self):
		...
		if ".git" not in os.listdir(os.getcwd()):
			sys.exit(
				Printer.MyPrinter().eprint("Git directory not found, please download Infoga from Github repository"))
	...
	def Start(self):
		if len(sys.argv) < 2:
			self.Usage()
		try:
			opts, args = getopt.getopt(self.argv, "t:s:i:",
		...
		for opt, arg in opts:
			if opt in ("-t", "--target"):
				...
				if self.engine not in ("all", "google", "bing", "yahoo", "pgp"):
					self.Banner()
					sys.exit(
						Printer.MyPrinter().eprint("Invalid search engine!! Try with: all,google, bing, yahoo or pgp"))
			elif opt in ("-i", "--info"):
				self.email = arg
